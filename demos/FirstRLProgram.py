...
def main():
    ...
    with task.start(async_mode=False, ctx=OnlineRLContext()):
        collector_env = BaseEnvManagerV2(
            env_fn=[lambda: DingEnvWrapper(gym.make('procgen:procgen-heist-v0')) for _ in range(cfg.env.collector_env_num)],
            cfg=cfg.env.manager
        )
        evaluator_env = BaseEnvManagerV2(
            env_fn=[lambda: DingEnvWrapper(gym.make('procgen:procgen-heist-v0')) for _ in range(cfg.env.evaluator_env_num)],
            cfg=cfg.env.manager
        )
